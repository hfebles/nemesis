        ...
        <tr id="tr_{{$i}}">
            ...
            <td id="td_{{$i}}" colspan="2" class="align-middle">
                <p class='align-middle mb-0' id='name_product{{$i}}'>
                    {{$products->name_product}} {{$products->name_presentation_product}} {{$products->short_unit_product}} @if ($products->tax_exempt_product == 1) (E) @endif
                </p>
            </td>
            ...
            <td class="text-center align-middle" width="10%">
                ...
                <p class='align-middle  mb-0' id='precio_productos_{{$i}}'>Bs. {{number_format($obj['precio_producto'][$i], 2, ',', '.')}}</p>
                <input type='hidden' name='precio_producto[]' id='price_product_{{$i}}' value="{{$obj['precio_producto'][$i]}}">
            </td>
            <td class="text-center align-middle " width="10%">
                <p class='align-middle  mb-0' id='subtotals_{{$i}}'>{{$obj['precio_producto'][$i]*$obj['cantidad'][$i]}}</p>
    ...
    <table class="table table-sm table-bordered mb-0"> 
        <tr>
            ...
            <th scope="col" class="text-end align-middle">
                <p class='align-middle mb-0' id="subFacs">Bs. {{number_format($data->no_exempt_amout_sales_order, 2, ',', '.')}}</p>
                <input type="hidden" id="subFac" name="subFac" value="{{$data->no_exempt_amout_sales_order}}">
        ...
        <tr>
            ...
            <th scope="col" class="text-end align-middle">
                <p class='align-middle mb-0' id="exentos">Bs. {{number_format($data->exempt_amout_sales_order, 2, ',', '.')}}</p>
                <input type="hidden" id="exento" name="exento" value="{{$data->exempt_amout_sales_order}}">
        ...
        <tr>
            ...
            <td class="text-end align-middle">
                <p class='align-middle mb-0' id="totalIVaas">Bs. {{number_format($data->total_amount_tax_sales_order, 2, ',', '.')}}</p>
                <input type="hidden" id="totalIVa" name="total_taxes" value="{{$data->total_amount_tax_sales_order}}">
            </td>
        </tr>
        <tr>
            ...
            <th scope="col" class="text-end align-middle">
                <p class='align-middle mb-0' id="totalTotals">Bs. {{number_format($data->total_amount_sales_order, 2, ',', '.')}}</p>
...
function addRow(){
    if (validateClient() == true){
        ...
        cell2.innerHTML = '<a id="search_productos_'+counterTR+'" onclick="abreModal(\'producto\', '+counterTR+')" class="btn btn-info btn-block mb-0"><i class="fas fa-search fa-lg"></i></a>';
        //cell2.id = "td_"+i
        ...
        cell4.innerHTML = "<input type='hidden' name='id_product[]' id='id_product_"+counterTR+"'><input onkeyup='calculate("+counterTR+", this.value)' class='form-control' autocomplete='off' id='cant_"+counterTR+"' type='number' name='cantidad[]'>";
        cell4.className="text-center align-middle"

        cell5.innerHTML = "<p class='align-middle  mb-0' id='precio_productos_"+counterTR+"'></p><input type='hidden' name='precio_producto[]' id='price_product_"+counterTR+"'>";
